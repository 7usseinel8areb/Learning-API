namespace ITI_API_Learn.Controllers
{
    ...
    public class DepartmentController : ControllerBase//response according status Code
    {
        ...
        [HttpGet("Emps/{id:int}")]
        public IActionResult GetDepartment1(int id)
        {
            Department department = _appDbContext.Departments.Include(dept => dept.Employee).FirstOrDefault(x => x.Id == id);
            DepartmentWithAllEmployeesDTO departmentWithAllEmployeesDTO = new DepartmentWithAllEmployeesDTO()
            {
            ...
            foreach(var item in department.Employee) {
                departmentWithAllEmployeesDTO.employees.Add(item.Name);
            }
            return Ok(departmentWithAllEmployeesDTO);
        }
